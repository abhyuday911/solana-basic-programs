...
describe("escrow case: cancel", () => {
  ...
  it("Cancels escrow", async () => {
    // get initial balance for later checks
    const initializerBalance = await provider.connection.getTokenAccountBalance(initializerTokenAccount);
    const initialVaultBalance = await provider.connection.getTokenAccountBalance(vault);

    await program.methods.cancelOffer().accounts({
      escrow: escrowKeypair.publicKey,
      vaultAuthority: vaultAuthority,
      vault: vault,
      initializerTokenAccount: initializerTokenAccount,
      tokenProgram: TOKEN_PROGRAM_ID
    }).signers([(provider.wallet as any).payer]).rpc()

    const finalInitializerBalance = await provider.connection.getTokenAccountBalance(initializerTokenAccount);
    assert.equal(finalInitializerBalance.value.amount, (BigInt(initializerBalance.value.amount) + BigInt(initialVaultBalance.value.amount)).toString())
  })
});